...
namespace GTAVisionUtils
{
    public class HashFunctions
    {
        ...
        public static void Draw3DLine(Vector3 iniPos, Vector3 finPos, byte col_r = 255, byte col_g = 255,
            byte col_b = 255, byte col_a = 255)
        {
            Function.Call(Hash.DRAW_LINE, iniPos.X, iniPos.Y, iniPos.Z, finPos.X, finPos.Y, finPos.Z, (int) col_r,
                (int) col_g, (int) col_b, (int) col_a);
        }

        public static void Draw3DBox(Vector3 pos, Vector3 size, byte col_r = 255, byte col_g = 255, byte col_b = 255,
            byte col_a = 255)
        {
            Function.Call(Hash.DRAW_BOX, pos.X - size.X / 2, pos.Y - size.Y / 2, pos.Z - size.Z / 2, pos.X + size.X / 2,
                pos.Y + size.Y / 2, pos.Z + size.Z / 2, (int) col_r, (int) col_g, (int) col_b, (int) col_a);
        }
        ...
        public static void DrawRect(float x, float y, float w, float h, byte r = 255, byte g = 255, byte b = 255,
            byte a = 255)
        {
            Function.Call(Hash.DRAW_RECT, x, y, w, h, (int) r, (int) g, (int) b, (int) a);
        }
        ...
        public static void Draw2DText(string text, float x, float y, byte r = 255, byte g = 255, byte b = 255,
            byte a = 255)
        {
